...
import api from "../api/api"; // axios instance with proper baseURL
...
export default function Events() {
  ...
  // ✅ Updated for Render — use relative paths
  const openFileInNewTab = (filePath) => {
    if (!filePath) return;
    const fullUrl = filePath.startsWith("/") ? filePath : `/${filePath}`;
      ...
      if (navigator.share) {
        if (ev.filePath) {
          try {
            ...
            console.warn("fetch for native file-share failed:", err);
          }
        }
